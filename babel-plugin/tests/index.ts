...
async function readTestCasesParallel() {
    ...
    const testCases = await Promise.all(testCaseRoots.map(async (root)=> {
        const [input, output, options ]= await Promise.all([
            fs.readFile(path.join(__dirname, `./fixtures/${root}/input.js`)),
            fs.readFile(path.join(__dirname, `./fixtures/${root}/output.js`)),
            fs.readFile(path.join(__dirname, `./fixtures/${root}/options.json`))
                .then(result => JSON.parse(result.toString()))
                .catch(_error => ({}))
        ]);
        return { fileId: root, input: input.toString(), output: output.toString(), options} as TestCase
    }));
